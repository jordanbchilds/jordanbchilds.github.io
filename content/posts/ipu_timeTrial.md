...
newer model, of CPU used as a base line comparison by [Kulkarni *et al.*](https://arxiv.org/abs/2012.14332). Kulkarni *et al.* showed that using the IPU can reduce computation time for approximate Bayesian Inference, for an SIR model, they however used PyTorch to implement their model. Here we wish to show that a significant speed up is still available when writing the code yourself, allowing for a wider range of more bespoke inference schemes and models. An introduction to executing C++ scripts on the IPU can be found on my "Beginners Guide to Executing C++ Scripts on an IPU" blog. A single GC200 chip, however, cannot be purchased and they can only be
