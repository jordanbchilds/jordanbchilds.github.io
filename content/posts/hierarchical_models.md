...
{{< rawhtml >}}
<p>Assume \(Y_i\) follows a binomial distribution, with probability of success \(\theta_i\), and the number of children in each school is known, then</p>
<p>\[
    Y_i \mid n_i, \theta_i \sim \text{Bin}\left(n_i, \theta_i\right).
\]</p>
<p>The parameters \(\theta_1, \theta_2, \dots, \theta_N\) are unknown and to be inferred. The pass rates are proportions, so it is reasonable to model them using a Beta distribution,</p>
<p>\[
    \theta_i \mid \alpha, \beta \sim \text{Beta}\left(\alpha, \beta \right)
\]</p>
<p>The hierarchy is implemented by considering the prior parameters \(\alpha\) and \(\beta\) unknown and, therefore, to be inferred. Both are continuous and positive; independent gamma distributions are one choice for the hyper-priors.</p>
<p>\[
\begin{aligned}
...
\end{aligned}
\]</p>
<p>The hyper-parameters \(\left\lbrace a_1, b_1, a_2, b_2 \right\rbrace\) are known and chosen by us to summarise prior beliefs. By inferring \(\alpha\) and \(\beta\), the distribution of a nationwide success rate can be inferred from the whole dataset. Which, in turn, affects the distribution of school-level success rates. The relationship can be seen by calculating the parameter FCDs up to proportionality.</p>
{{< /rawhtml >}}

## Parameter inference

{{< rawhtml >}}
<p>First, consider the FCDs of the prior parameter \(\alpha\),</p>
<p>\[
\begin{aligned}
    p(\alpha \mid \beta, \boldsymbol{\theta}, \boldsymbol{y}) &\propto p\left(\alpha, \beta, \boldsymbol{\theta}, \boldsymbol{y}\right), \\
    &\propto p(\boldsymbol{y} \mid \boldsymbol{\theta}) p (\boldsymbol{\theta} \mid \alpha, \beta) p(\alpha) p(\beta), \\
    &\propto p(\boldsymbol{\theta} \mid \alpha, \beta) p(\alpha), \\
    &\propto \prod_{i=1}^n \left\lbrace {\theta_i}^{\alpha-1} \left( 1 - \theta_i\right)^{\beta-1}\right\rbrace \times \alpha^{a_1 -1} e^{-b_1 \alpha}, \\
    &\propto \alpha^{a_1 -1} e^{-b_1 \alpha} \prod_{i=1}^n{\theta_i}^{\alpha-1}.
\end{aligned}
\]</p>
<p>Unfortunately, this is not the form of a known probability distribution, and therefor the analysis is not semi-conjugate. Despite this, inference can still proceed by implementating MCMC methods, see <a href="https://jordanbchilds.github.io/posts/mcmc_introduction/">here</a>. Nevertheless, it can be seen that given \(a_1, b_1\) and \(\boldsymbol{\theta}\), \(p(\alpha \mid \beta, \boldsymbol{\theta}, \boldsymbol{y})\) is independent of \(\beta\) and \(\boldsymbol{y}\). Similarly, calculating the FCD of \(\beta\) shows that given \(a_2, b_2\) and \(\boldsymbol{\theta}\), it is independent of the observations \(\boldsymbol{y}\) and \(\alpha\).</p>
<p>\[
    p(\beta \mid \alpha, \boldsymbol{\theta}, \boldsymbol{y})
     \propto \alpha^{a_\beta -1} e^{-b_\beta \beta} \prod_{i=1}^n\left( 1 - \theta_i\right)^{\beta-1}
\]</p>
<p>The final parameters to consider are the individual school success rates</p>
<p>\[
\begin{aligned}
    p(\theta_i \mid \alpha, \beta, \boldsymbol{\theta}_{-i}, \boldsymbol{y}) &\propto p\left(\alpha, \beta, \boldsymbol{\theta}, \boldsymbol{y}\right), \\
    &\propto p(\boldsymbol{y} \mid \boldsymbol{\theta}) p (\boldsymbol{\theta} \mid \alpha, \beta), \\
    &\propto \prod_{i=1}^n \left\lbrace {\theta_i}^{y_i} \left( 1 - \theta_i\right)^{n_i - y_i}\right\rbrace \times \prod_{i=1}^n\left\lbrace {\theta_i}^{\alpha-1} \left(1 - \theta_i\right)^{\beta -1}\right\rbrace, \\
    &\propto {\theta_i}^{y_i + \alpha - 1} \left(1 - \theta_i\right)^{n_i - y_i + \beta - 1}.
\end{aligned}
\]</p>
<p>This is the form of a Beta distribution with parameters \(y_i+\alpha\) and \(n_i - y_i + \beta\), and, therefore,</p>
<p>\[
    \theta_i \mid \alpha, \beta, y_i \sim \text{Beta} \left(y_i+\alpha, n_i - y_i + \beta \right).
\]</p>
<p>The FCD of the individual school success rates depends on the observed data of only that school, \(y_i\), and the population-level parameters, \(\alpha\) and \(\beta\).</p>
{{< /rawhtml >}}
